...
import {deleteCast, getCasts, saveCasts} from "../../apis/casts";
...
function CastPage() {
    ...
    function onSubmit(e: React.FormEvent) {
        ...
        if(performanceID) saveCasts({performanceID, casts}).then((res) => {
            if(res.status === 200) setAlarm("저장되었습니다")
            refreshCharacters()
        })
    }
                            ...
                            <form className={"add-cast-form"} onSubmit={onSubmit}>
                                {(casts && casts.length>0) && casts.map((cast) =>
                                    <div key={cast.idx} data-idx={cast.idx}>
                                        ...
                                        <select name={"character"} defaultValue={cast.characterID} onChange={formCharacterChange}>
